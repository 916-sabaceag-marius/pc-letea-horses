...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadRestaurants() {
      ...
      const mockRestaurants = [
        { id: "RESTO_001", name: "Downtown Diner" },
        { id: "RESTO_002", name: "Pizza Palace" },
        { id: "RESTO_003", name: "Sushi Central" },
        { id: "RESTO_004", name: "Burger Hub" },
        { id: "RESTO_005", name: "Cafe Delight" }
      ];
      setRestaurants(mockRestaurants);
      const defaultId = mockRestaurants[0]?.id || null;
      setSelectedRestaurant(defaultId);
      setRestaurantId(defaultId);
    };
  ...
  if (!data || !data.succeeded) return <p>Loading...</p>;
      ...
      tooltip: {
        callbacks: {
          label: (tooltipItem) => {
            ...
            return `${cat.name}: ${cat.value} sales`;
          }
        }
      }
    }
  ...
  const doughnutData = {
    ...
    datasets: [
      {
        data: data.data.categories.map(c => c.value),
        backgroundColor: data.data.categories.map((_, idx) => tailwindColors[idx % tailwindColors.length]),
      }
    ]
  };
    ...
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden">
      <div className="flex-1 px-4 sm:px-6 lg:px-10 py-8">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          ...
          <div className="relative">
            <button
              ...
              className="flex justify-between items-center h-9 px-3 rounded-md border bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200"
            >
            ...
            {showRestaurantDropdown && (
              <ul className="absolute mt-1 bg-white dark:bg-gray-800 border rounded shadow z-10 w-full max-h-60 overflow-auto">
                {restaurants.map(r => (
                  <li
                    key={r.id}
                    className="px-3 py-2 cursor-pointer hover:bg-blue-200 dark:hover:bg-blue-800"
                    onClick={() => {
        ...
        <div className="flex gap-2 p-1  bg-gray-100 dark:bg-gray-800 rounded-lg">
          {['lastDay', 'lastWeek', 'lastMonth', 'lastQuarter'].map((r) => (
            <button
              key={r}
              className={`flex h-9 items-center justify-center px-4 rounded-md ${r === range ? 'bg-blue-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'
                }`}
              onClick={() => {
                setRange(r);
                setShowCustom(false);
          ...
          {range === 'customRange' ? (
            ...
            <button
              key="customRange"
              className={`flex h-9 items-center justify-center px-4 rounded-md ${range === 'customRange' ? 'bg-blue-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}
              onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-20 mb-6">
        {[
          ...
          <div key={stat.title} className="flex flex-col items-center gap-2 p-6 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        {/* Line chart */}
        <div className="lg:col-span-2 p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl">
          <p className="text-lg font-semibold mb-4">Sales Over Time</p>
          <Line data={lineChartData} />
        </div>

        {/* Doughnut chart */}
        <div className="flex flex-col p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                <th className="py-3 px-4 text-sm font-semibold text-gray-500 dark:text-gray-400">Product</th>
                <th className="py-3 px-4 text-sm font-semibold text-gray-500 dark:text-gray-400 text-right">Total Items Sold</th>
              </tr>
            </thead>
            <tbody>
              {data.data.topProducts.map((prod) => (
                <tr key={prod.name} className="border-b border-gray-200 dark:border-gray-700">
                  <td className="py-3 px-4 flex items-center gap-4">
                    <img className="w-12 h-12 rounded-md object-cover" src={prod.img} alt={prod.name} />
                    <div>
                      <p className="font-semibold">{prod.name}</p>
                      <p className="text-sm text-gray-400 dark:text-gray-500">{prod.category}</p>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-right font-medium">{prod.sales}</td>
                </tr>
              ))}
            </tbody>
          </table>
